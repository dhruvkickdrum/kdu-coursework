...
import { fetchAllProducts, searchProducts } from '../redux/productSlice'

export default function Home() {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const { products, searchQuery, loading, searchLoading, error} = useAppSelector((state) => state.products);
  ...
  const handleRetry = useCallback(() => {
    if(isSearching) {
      void dispatch(searchProducts(searchQuery.trim()));
      return;
    }
